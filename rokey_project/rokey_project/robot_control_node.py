...
class TaskStatePublisher(Node):
    def __init__(self):
        super().__init__('task_state_publisher')
        self.publisher = self.create_publisher(TaskState, '/task_state', 10)

    def publish_state(self, state, obj=""):
        msg = TaskState()
        msg.state = state
        msg.object_name = obj
        self.publisher.publish(msg)
        self.get_logger().info(f"Published: state={state}, object={obj}")
...
'''인사하는 함수'''
def greeting(vel, acc):
    print('로봇이 인사를 시작합니다!')
    movej([0, 0, 90, 0, 90, 0], vel=vel, acc=acc)
    movej([45.0, 0, 90, 0, -90, 0], vel=vel, acc=acc)
    release()
    move_periodic(amp =[0, 0, 60, 0, 7, 0], period=[0, 0, 6, 0, 1.2, 0], atime=0.5, repeat=2, ref=DR_TOOL)
    grip()
    movej([0, 0, 90, 0, 90, 0], vel=vel, acc=acc)


'''로봇을 칠 때 까지 대기하는 함수'''
def robot_wait(vel, acc):
    ...
    if put_cnt==0 or out_cnt==0:
        ...
        while True:
            ...
            if val_y == 0:
                ...
                print("수납을 시작합니다.")
                publisher.publish_state(state = "good_night")
            ...
            ## x 방향으로 쳤을 때 꺼내기 시작
                ...
                print(f"check_force_condition Y: {val_y}, X: {val_x}")
                print("꺼내기를 시작합니다.")
                publisher.publish_state(state = "start")
                rclpy.spin_once(publisher)
                greeting(vel, acc)
                outputs = get_user_inputs_out()
                inputs = []
                print(f'outputs = {outputs}')
                out_true=True
                out_cnt+=1
                break
    return put_true, out_true, inputs, outputs


'''물체명, 적재 위치 지정하는 함수'''
def get_user_inputs():
    print("물체명과 위치 번호를 입력하세요 (예: 'tumblr 1').\n엔터만 입력하면 종료됩니다.\n")
    print('(목록: tumblr, snack, gum, wallet, cardkey)')
    inputs = []
    while True:
...
'''물체명, 꺼내는 위치 지정하는 함수'''
def get_user_inputs_out():
    print("물체명과 위치 번호를 입력하세요 (예: 'tumblr 1').\n엔터만 입력하면 종료됩니다.\n")
    print('(목록: tumblr, snack, gum, wallet, cardkey)')
    outputs = []
    while True:
        line = input("꺼낼 물건: ").strip()
        if line == "":
            break
        outputs.append(line)
    return outputs
...
def find_object(vel, acc):
    publisher.publish_state(state = "searching")
    rclpy.spin_once(publisher)
    # movel([162.30, 305.92, 100.00, 53.48, -179.14, 53.20], vel=vel, acc=acc) # 탐색 초기 위치
    movel([210.00, 305.92, 100.00, 53.48, -179.14, 53.20], vel=vel, acc=acc) # 탐색 초기 위치
    movel([0, 0, -97, 0, 0, 0], vel=vel, acc=acc, mod=DR_FC_MOD_REL) # -95만큼 내려감
    cnt=0
    i=1
    new_pos = []
...
def grap_object(new_pos, i, height_dict, HEIGHT_RANGE):
    ...
    movel([0, 0, 215, 0, 0, 0], vel=60, acc=60,mod=1)
    if i%2!=0:
    ...
    if height_dict['tumblr'] - HEIGHT_RANGE < pos < height_dict['tumblr'] + HEIGHT_RANGE: #
        # tumblr일 때 20mm 올리고 70mm 내림
        detected_name = 'tumblr'
    ...
    elif height_dict['snack'] - HEIGHT_RANGE < pos < height_dict['snack'] + HEIGHT_RANGE: #
        # snack일 때 20mm 올리고 70mm 내림
        detected_name = 'snack'
        print(f'detect = {detected_name}')
        movel([0, 0, 20, 0, 0, 0], vel=60, acc=60,mod=1)
        release()
    ...
    elif height_dict['gum'] - HEIGHT_RANGE < pos < height_dict['gum'] + HEIGHT_RANGE: #
        # gum일 때 20mm 올리고 70mm 내림
        detected_name = 'gum'
        print(f'detect = {detected_name}')
        movel([0, 0, 20, 0, 0, 0], vel=60, acc=60,mod=1)
        release()
    ...
    elif height_dict['wallet'] - HEIGHT_RANGE < pos < height_dict['wallet'] + HEIGHT_RANGE: #
        # wallet일 때 20mm 올리고 67mm 내림
        detected_name = 'wallet'
        print(f'detect = {detected_name}')
        movel([0, 0, 20, 0, 0, 0], vel=60, acc=60,mod=1)
        release()
    ...
    elif height_dict['cardkey'] - HEIGHT_RANGE < pos < height_dict['cardkey'] + HEIGHT_RANGE:
        # 카드키일 때 20mm 올리고 40mm 내림
        detected_name = 'cardkey'
        movel([0, 0, 20, 0, 0, 0], vel=60, acc=60,mod=1)
        release()
    ...
    publisher.publish_state(state = "grapped", obj = f'{detected_name}')
    rclpy.spin_once(publisher) 
    return pos
...
def place_object(detected_name, name, index, J_grip_home, J_grip_right, J_grip_left, place_list, stacked, place_dict_posx):
    ...
    time.sleep(2)
    publisher.publish_state(state = "placed", obj = f'{detected_name}')
    rclpy.spin_once(publisher)  
    ...
    if index == 2 or index == 4:
        movel([0, 40, 200, 0, 0, 0], vel=60, acc=60, mod=1)
...
'''물건 꺼내기 동작을 하는 함수'''
def out_object(vel, acc, place_list, place_dict, J_grip_home, J_grip_right, J_grip_left, stacked, place_dict_posx, outputs):

    # out_place_list_x = [[277.25, 216.84], [427.80, 216.62], [276.85, 68.43], [426.63, 66.08], [425.55, -83.28]]
    out_place_list_x = [[574.39, 63.46], [574.13, -83.83], [425.94, 68.34], [423.88, -83.04], [420.32, -232.99]]

    for i, output in enumerate(outputs):
        name, index = output.split()
        index = int(index)
        print(f"'{name}' 를 저장된 위치 {index}에서 가져옵니다")
        publisher.publish_state(state = "out", obj = f'{name}')
        rclpy.spin_once(publisher)
        ...
        if name == 'tumblr':
            ...
            # cardkey일 때 20mm 올리고 40mm 내림
            detected_name = 'cardkey'
            print(f'detect = {detected_name}')
            movel([0, 0, 20, 0, 0, 0], vel=60, acc=60,mod=1)
            release()
            movel([0, 0, -40, 0, 0, 0], vel=60, acc=60,mod=1)
            # cardkey는 compliance 사용하여 집음
            task_compliance_ctrl(stx=[500, 500, 500, 100, 100, 100])
            time.sleep(1)
            grip()
            release_compliance_ctrl()
        ...
        try:
            ...
            print("try [current_posx = get_current_posx()[0]]")
        except:
            print("except [current_posx = get_current_posx()[0]]")
            time.sleep(3)
            current_posx = get_current_posx()[0]
        time.sleep(1)
        ...
        time.sleep(0.3)
        movel(current_posx, vel=vel, acc=acc)
        time.sleep(0.3)
        ...
        time.sleep(1)
        release()
        movel([0, 0, 100, 0, 0, 0], vel=vel, acc=vel,mod=1)
        movej([0, 0, 90, 0, 90, 0], vel=vel, acc=acc)

    print("입력한 물체를 모두 꺼냈습니다!")
    outputs = []
...
def main(args=None):
    ...
    place_dict = {}
    place_dict_posx = {}
    height_dict = {'tumblr': 197, 'snack': 96.42, 'gum': 80, 'wallet': 30, 'cardkey': 6}
    HEIGHT_RANGE = 10
    ...
    while rclpy.ok(): # 구동부
        ...
        ## =============== 로봇 쳤을 때 동작 시작 ===============
        put_true, out_true, inputs, outputs = robot_wait(vel=VELOCITY, acc=ACC)  # 대기중, y 방향으로 쳤을 때 수납, x 방향으로 쳤을 때 꺼내기 시작
        ...
        if out_true:
            ...
            # 수납된 물체가 있을 때
                ...
                stacked, outputs = out_object(VELOCITY, ACC, place_list, place_dict, J_grip_home, J_grip_right, J_grip_left, 
                                              stacked, place_dict_posx, outputs)  # 물건 꺼내기 동작을 하는 함수
        publisher.publish_state(state = 'done')
        rclpy.spin_once(publisher)
    rclpy.shutdown()
