...
Release 후  물체 grip
10. 입력 위치에 물품 유무 확인 후 해당 위치에 수납 -1. 해당 위치에 물건이 없을 경우
 -> 원래 위치 placed_list에 수납 
2.해당 위치에 이미 물건이 수납된 경우 
 ex) stacked [1,0,0,0] -> stacked [2,0,0,0]
 원래 위치 에서 x축 으로 떨어진 지점에 수납
순응제어로 물체 놓기 후 그리퍼 홈으로 이동
