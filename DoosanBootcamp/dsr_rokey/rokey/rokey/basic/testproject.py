                    ...
                    if detected_name:
                        for item in inputs:
                            ...
                            if name == detected_name:
                                ...
                                stacked=False
                                for old_item in old_inputs:
                                    old_name, old_index = old_item.split()
                                    old_index = int(old_index)
                                    if  old_index == index:
                                        print("이미 물체가 수납되었습니다.")
                                        joint_pose = place_list[index-1]
                                        tcp_pose = fkin(joint_pose)
                                        tcp_pose[0] += 150
                                        movel(tcp_pose, vel=30, acc=30, mod=0)
                                        new_place_dict[index]=tcp_pose
                                        stacked=True
                                        break
                                ...
                                item_to_save = inputs[0]
                                inputs.pop(0)
                                old_inputs.append(item_to_save)
                ...
                if not place_dict:
                    ...
                    for item in inputs:
                        ...
                        if name not in place_dict:
                            print(f"'{name}'는 저장된 물체가 아닙니다.")
                            continue

                        saved_index = place_dict[name]
                        if saved_index != index:
                            print(f"'{name}'는 {saved_index}번 위치에 저장되어 있으며 요청한 위치 {index}와 다릅니다.")
                            continue

                        print(f"'{name}' 를 저장된 위치 {index}에서 가져옵니다")
                        release()

                        if index in new_place_dict:
                            print("new_place_dict에 있는 위치로 이동합니다")
                            movel(new_place_dict[index], vel=30, acc=30, mod=0)
                        else:
                            movej(place_list[index-1], vel=30, acc=30, mod=0)
