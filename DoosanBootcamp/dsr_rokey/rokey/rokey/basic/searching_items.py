...
from DSR_ROBOT2 import set_digital_output, get_digital_input, amovel,get_current_posx,mwait,movel,movej,task_compliance_ctrl,set_desired_force,DR_FC_MOD_REL,check_force_condition,release_force,release_compliance_ctrl,DR_AXIS_Z

def grip():
    set_digital_output(1, 1)
    set_digital_output(2, 0)
    # 그리퍼 닫힘get_digital_input
    time.sleep(1)
...
def main(args=None):
    ...
    place_list = [ [-107.50,41.73,42.56,-0.16,99.45,-107.83],[-112.82,25.04,102.76,-0.15,52.24,-112.60], [-68.84,26.89,100.16,1.37,55.58,-69.53],[-75.05,42.44,41.49,1.08,99.17,-74.33]]
    ...
    while rclpy.ok(): # 구동부
        ...
        if put_cnt==0 or out_cnt==0:
            ...
            while True:
                if check_force_condition(DR_AXIS_Y, 10,30,DR_TOOL)==0:
                    val_y = check_force_condition(DR_AXIS_Y, 10, 30, DR_TOOL)
                    val_x = check_force_condition(DR_AXIS_X, 10, 30, DR_TOOL)
                    print(f"check_force_condition Y: {val_y}, X: {val_x}")
                        #fx, fy, fz = get_tool_force()[:3]       
                    ...
                    put_cnt+=1
                    break
                elif check_force_condition(DR_AXIS_X, 10,30,DR_TOOL)==0:
                    val_y = check_force_condition(DR_AXIS_Y, 10, 30, DR_TOOL)
                    val_x = check_force_condition(DR_AXIS_X, 10, 30, DR_TOOL)
                    print(f"check_force_condition Y: {val_y}, X: {val_x}")
                    #fx, fy, fz = get_tool_force()[:3]       
        ...
        while put_true:
            ...
            movel([162.30, 305.92, 100.00, 53.48, -179.14, 53.20], vel=VELOCITY, acc=ACC)
            cnt=0
            i=1
            ...
            while cnt<9 and not force_triggered:
                ...
                if i%2 != 0:
                    ...
                    #x=amovel([400, 0, 0, 0, 0, 0], vel=60, acc=60, mod=1)
                    i+=1
                else:
                    amovel([-400, 0, 0, 0, 0, 0], vel=60, acc=60, mod=1)
                    #x=amovel([-400, 0, 0, 0, 0, 0], vel=60, acc=60, mod=1)
                ...
                if detected_name:
                    for item in inputs:
                        ...
                        if name == detected_name:
                            ...
                            movej([-79.50,7.48,85.07,0.01,101.23,-79.72],vel=30, acc=30,mod=0)
